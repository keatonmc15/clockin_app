...
useEffect(() => {
  ...
  async function clockOut(allowOutsideGeofence = false) {
    if (!loggedIn) return;
    const permissionOk = await ensureLocationReadyForClockIn();
    if (!permissionOk) return;
    ...
    if (!res.ok) {
      ...
      if (res.status === 409 && data?.error === 'outside_store_geofence' && data?.requires_confirmation) {
        Alert.alert('Outside store location', 'You appear to be outside the store location. Clock out anyway?', [
          {text: 'Not now', style: 'cancel'},
          {text: 'Clock Out', style: 'destructive', onPress: () => clockOut(true)},
        ]);
        return;
      }
  ...
  function bgEventContext() {
    const currentOpenShift = latestBgRef.current.status?.open_shift || status?.open_shift || null;
    if (!latestBgRef.current.loggedIn || !currentOpenShift) return {};

    return {
      employee_code: String(latestBgRef.current.employeeCode || '').trim().toLowerCase(),
      shift_id: currentOpenShift.shift_id,
      store_id: currentOpenShift.store_id,
    };
  }
    ...
    const subGeofence = BackgroundGeolocation.onGeofence(async (event: GeofenceEvent) => {
      ...
      try {
        ...
        await postBgEvent({
          ...
          lat: gfCoords.latitude,
          lng: gfCoords.longitude,
          accuracy_m: gfCoords.accuracy,
          ...bgEventContext(),
